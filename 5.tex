\chapter{Exponential integration methods in JAGUAR}
  ...
  \section{Analysis of exponential time integration methods}
    ...
    \subsection{Order: convected inviscid isentropic vortex}
      ...
      After 20 periods, we can compute the error for a variable $u\left(\vec{x}, t\right)$:
      \begin{equation}
        \operatorname{err}\left(u\right) = \int_{\left[0, L\right]^2} \norm[2]{u\left(\vec{x}, 20T\right) - u\left(\vec{x}, 0\right)} \mathrm{d}\vec{x} \ .
      \end{equation}
      ...
      \end{equation}
      with a $K$-Lipschitz continuous increment function $g$, we have \PS{Suli, E., and Mayers, D. F. (2003). An introduction to numerical analysis. Cambridge University Press.}:
      \begin{equation}
      ...
      \PS{Mais du coup on va montrer qu'on a le bon ordre sur l'errreur globale, ca n'implique rien sur l'ordre de la méthode ?????????}
