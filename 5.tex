...
\end{tcolorbox}
  ...
  \section{Analysis of exponential time integration methods}
    ...
    \subsection{Order analysis: convected inviscid isentropic vortex}
      ...
      \PS{For this application, the Spectral difference method uses the approximate Riemann solver of Roe.}
      \footnote{\GP{Quel solveur de Riemann ???? Tu ne l'as pas dit...}}
    ...
    \subsection{Robustness analysis: Taylor--Green vortex}
      ...
      \PS{Figure \ref{fig:tgv_fields} shows contours for which the second invariant of the velocity gradient, also called Q-criterion, is equal to 0.1.
      This was chosen as it roughly indicates the sizes of turbulences structures.}

      \begin{figure}
        \centering
        \includegraphics[width=\textwidth]{figures/tgv_fields.png}
        \caption{\PS{0.1 contours of the Q-criterion coloured by velocity norm of the Taylor--Green vortex at various times.}}
      ...
      \PS{The Spectral difference method uses the approximate Riemann solver of Roe.}
      ...
      \PS{As a simulation corresponds to the computation with the largest time step for which the results are satisfying, this figure does not give much information.
      ...
      They all give the same result, which is the exact solution minus the error from the spatial discretisation method.
    ...
    \subsection{Industrial application: LS89}
      ...
      Indeed, it seems to be the quickest method, and we wich here to reduce the computational time.
      The other exponential methods would improve the results accuracy, so we keep only the Rosenbrock--Euler method to simplify the analysis.
      }
      ...
      It was originally true at the time of some previous investigations \cite{BrunetCronerMinotEtAl2018}, but as the diffusive scheme was greatly improved \PS{REF des travaux de Hugues ?}, it is not true anymore.
