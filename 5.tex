\chapter{Exponential integration methods in JAGUAR}
  ...
  \section{Analysis of exponential time integration methods}
    ...
    \begin{itemize}
      ...
      \item RK3SHU, a 3 stage third-order TVD method from \cite{ShuOsher1988, GottliebShu1996}
      \item SSPRK(5, 4), an optimal SSP 5 stage fourth-order method from \cite{SpiteriRuuth2002}.
    \end{itemize}
    Low-storage methods are Runge--Kutta methods for which the matrix $A$ in their Butcher tableau is lower diagonal, or in other words all $a_{ij}$ are null except for $j = i - 1$.
    ...
    The last two methods are Runge--Kutta methods with an additional property first called TVD \cite{GottliebShu1996} and more recently SSP  \cite{GottliebShuTadmor2001}.
    ...
    It means they are defined differently than other methods, using the coefficients $\alpha_{0 < i \leq k, 0\leq j < i}$ and $\beta_{0 < i \leq k, 0\leq j < i}$ as:
    \begin{equation}
      \left\{\begin{aligned}
    ...
    \subsection{Order: convected inviscid isentropic vortex}
      ...
      \end{equation}
      with a $K$-Lipschitz continuous increment function $g$, we have \cite{SueliMayers2003}:
      \begin{equation}
      ...
      What interest us is the global truncation error, as it is the error a user look at, and this is why we look at it in this test case.
      \PS{Mais du coup on va montrer qu'on a le bon ordre sur l'erreur globale, ca n'implique rien sur l'ordre de la méthode ?????????}
