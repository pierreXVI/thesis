...
\end{tcolorbox}
  ...
  \section{JAGUAR: a Spectral Difference solver}
    ...
    \subsection{The Spectral Difference method}
      ...
      For \GP{any polynomial degree $p-1$ of the solution}, the proposed Spectral Difference method is naturally of order $p$.
      Indeed, the method can represent exactly any polynomial of degree $p\!-\!1$ and, as a consequence, the error term is of order $p$.
      In JAGUAR, the \GP{polynomial} order $p$ ranges from 2 to 10, but $2 \le p \le 6$ for practical applications.
  ...
  \section{Analysis of exponential time integration methods}
    ...
    \subsection{Order analysis: convected inviscid isentropic vortex}
      ...
      \GP{The period for those numerical values is $L / U_\infty \simeq \num{0.000576}\si{\second}$.}
    ...
    \subsection{Robustness analysis: Taylor--Green vortex}
      ...
      As a simulation correspond to the computation with the largest time step for which the results are satisfying, this figure does not give much information: \GP{all methods produce results that are close to reference data}.
      ...
      In the following, \GP{the numerical solution provided by the workshop is assumed to be the exact solution, even if it is a numerical solution and not an algebraic one.}\footnote{\PS{pas compris la phrase}}\GP{C'est mieux ?}
      %Je vire : dit plus haut dans le \GP... \PS{Apart from this error, all curves are all gathered around the same solution.}
