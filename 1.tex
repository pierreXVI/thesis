\chapter{Analysis of existing methods}
  ...
  \section{Brief introduction to the spatial integration schemes}
    ...
    \subsection{Gradient reconstruction methods}
      ...
      The choices made for the spatial discretisation methods are motivated by the fact that CEDRE and in particular the fluid solver CHARME aim to work with \PS{general unstructured meshes}.
  ...
  \section{Introduction to the time integration methods}
    ...
    \subsection{Analysis of time integration methods}
      ...
      \subsubsection{Consistency and order}
        ...
        To convince oneself, we could say that when solving the differential equation \PS{on/in} an interval $\left[0, T\right]$ with a fixed $T$, the global error of a $p$-order method would \PS{be of, in, behave like ?} $O\left(\Delta t^p\right)$ as it amounts to summing $T/\Delta t$ local errors of $O\left(\Delta t^{p+1}\right)$.
    ...
    \subsection{Explicit methods}
      ...
      \subsubsection{Adams--Bashforth methods}
        ...
        The stability analysis for such methods is a bit more complex \cite{HairerNorsettWanner1993, HairerWanner1996}, and so we show the result that we obtained numerically on figure \ref{fig:ab_stab} without \PS{faire le calcul}.
      ...
      \subsubsection{Implicit Runge--Kutta methods}
        ...
        \begin{equation}
          \left\{\begin{aligned}
            ...
            \textrm{with}\quad \operatorname{G}_{n,i} &= \operatorname{G}\left(\Xi_n - \Delta t \sum_{j = 1}^{i-1} \alpha_{ij} \operatorname{G}_{n,j}\right) - \frac{\mathrm{d} \operatorname{G}}{\mathrm{d} \Xi}\left(\Xi_n\right) \sum_{j = 1}^{i} \gamma_{ij} \operatorname{G}_{n,j}
          \end{aligned}\right. \ .
        \end{equation}
        ...
        It also introduces the additional parameters $\gamma_{ij}$ to add more flexibility to the method \cite{Wanner1977}.
        To be more precise, this is sometimes called a Rosenbrock--Wanner method, and classical Rosenbrock methods corresponds to when all $\gamma_{ij}$ are null, except for the $\gamma_{ii}$, that correspond to the $a_{ii}$ from the DIRK method.
        However, the distinction is not made very often, and Rosenbrock--Wanner methods are simply called Rosenbrock methods.
        The method is defined by the coefficients $\alpha_{ij, 1\leq j<i\leq k}$, $b_{i, 1\leq i\leq k}$ and $\gamma_{ij, 1\leq j\leq i\leq k}$.
        If the problem is non-autonomous, the method is:
        \begin{equation}
          \left\{\begin{aligned}
            \Xi_{n+1} &=    \Xi_n - \Delta t \sum_{i = 1}^k b_i \operatorname{G}_{n,i} \\
            \textrm{with}\quad \operatorname{G}_{n,i} &=
              \operatorname{G}\left(\Xi_n - \Delta t \sum_{j = 1}^{i-1} \alpha_{ij} \operatorname{G}_{n,j} \ ,\  t_n + c_i \Delta t\right)\\
              &\phantom{= {}} -\frac{\partial \operatorname{G}}{\partial \Xi}\left(\Xi_n, t_n\right) \sum_{j = 1}^{i} \gamma_{ij} \operatorname{G}_{n,j} \\
              &\phantom{= {}} -\frac{\partial \operatorname{G}}{\partial t}\left(\Xi_n, t_n)\right) \sum_{j = 1}^i \gamma_{ij} \Delta t
          \end{aligned}\right.
        \end{equation}
  ...
  \PS{Revoir structure}
    ...
    \subsection{Evaluating the Jacobian matrices}
      ...
      \end{equation}
      that gives a smaller error in \PS{article (in, of, ...)} $o\left(\varepsilon^2\right)$, but it requires two $f$ evaluations, so it is twice as expensive as the first-order approximation.
