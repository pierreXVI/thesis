...
\begin{tcolorbox}[title=Résumé du chapitre : Analyse des méthodes existantes, colframe=black!50!white]
  \paragraph{}
  Le but de ce chapitre est de choisir les méthodes numériques qui serviront pour la suite.
  Il s'agit d'identifier les faiblesses de l'intégration temporelle de CEDRE, et d'exhiber des méthodes pouvant les corriger tout en étant compatibles avec l'existant.

  \paragraph{}
  Tout d'abord, nous introduisons un contexte mathématique et ses notations.
  Nous décrivons ensuite comment un solveur passe d'une équation aux dérivées partielles, issue du modèle physique, à une équation différentielle ordinaire, purement numérique.
  Nous introduisons donc brièvement les méthodes de discrétisation spatiale, et en particulier les deux principales du solveur CEDRE.
  Grâce à ces méthodes et à l'utilisation d'un maillage, nous arrivons donc à l'équation différentielle ordinaire qu'il nous faut intégrer avec des méthodes d'intégration temporelle.

  \paragraph{}
  Nous introduisons ensuite des outils pour caractériser de telles méthodes, en définissant les notions d'ordre et de stabilité.
  Nous rappelons les méthodes d'intégrations que l'on trouve classiquement dans la littérature, en soulignant la dichotomie explicite/implicite.

  \paragraph{}
  Puisque nous nous intéressons aux méthodes d'intégration implicites, pour des raisons que nous rappelons dans ce chapitre, nous détaillons les différents éléments intervenant dans sa mise en place : le solveur non-linéaire, le solveur linéaire, et en particulier le passage de l'un à l'autre.
  Ce passage étant jugé insatisfaisant pour notre solveur, nous présentons la méthode JFNK, ou plus généralement une formulation sans matrice, qui s'appuie sur des parties déjà présentes de CEDRE.
  Ce choix est motivé par le fait que la Jacobienne traditionnellement utilisée est très approximée car elle ne prend pas en compte les méthodes de reconstruction du schéma MUSCL et les modèles physiques plus complexes comme la turbulence.
\end{tcolorbox}
  ...
  \section{Problem setup}
    ...
    \PS{This relation is quite simple, as it is the one that corresponds to ideal gas with constant heat capacity.
    In our solver, this relation is usually more complex as we are working with multifluid flows and with variable heat capacities.}
    \footnote{\LM{Si tu en es à dire qu'il y a plusieurs modèles pour $\tau$, n'hésite pas à signaler que la fermeture que tu exhibes ici pour $\rho E$ est la plus simple possible (gaz parfait cp constant)... Elle n'est en pratique que rarement utilisée...}}
  ...
  \section{Brief introduction to the spatial integration schemes}
    ...
    \subsection{The Finite Volume method}
      ...
      In a closed domain where nothing comes in or out, they are indeed conserved.
  ...
  \section{Introduction to the time integration methods}
    ...
    \subsection{Analysis of time integration methods}
      ...
      \subsubsection{Stability}
        ...
        Finally, this is why we study equation (\ref{eq:dahlquist}) assuming the eigenvalues of $J$ are negative. \LM{On comprend bien ce que tu veux dire, mais, n'y a-t-il pas un terme plus précis pour qualifier la solution $\tilde{\Xi}$, du genre qu'elle est linéairement stable, ou autre chose? Si non, ben tant pis c'est pas grave... }
        ...
        \paragraph{}
        The key property resulting in the stability analysis of time integration methods is the \emph{A-stability} \cite{Dahlquist1963}. \LM{Point de déta  il, mais ici tu ne t'embarrasse pas à considérer les valeurs propres de J, ni à décomposer $y$ en vecteurs propres, faut-il l'évoquer , juste pour bien lier $z$ et valeur propre de $\Delta t J$?}
    ...
    \subsection{Implicit methods}
      ...
      \subsubsection{Implicit Runge--Kutta methods}
        ...
        \begin{equation}
          ...
          \end{aligned}\right.
        \end{equation}
        with $c_i = \sum_{j = 1}^{i - 1} \alpha_{ij}$ as for other Runge--Kutta methods. \LM{Du coup, j'ai du rater la définition des $\alpha_{ij}$, ou c'est une typo?}
  ...
  \section{Implicit methods framework}
    ...
    \subsection{Linear solver}
      ...
      For the spatial discretisation methods we use, a cell only depends on a small number of neighbouring cells, and therefore a degree of freedom is not linked with most of the others \PS{(c'est clair ?)} \LM{Il me semble... Enfin, j'ai compris quoi...}.
      ...
      The cleverness resides in the fact that those Krylov subspaces are \PS{emboités} \LM{nested (one in another)?}, and each iteration reuses the information obtained in the previous ones.
      ...
      As the matrices we encounter in our computational fluid dynamics problems are \PS{quelconques} \LM{arbitrary ?}, meaning \PS{à priori non-normales}\LM{mostly nonnormals ?}, we decided not to study finely the spectrum of the operator.
      ...
      One of the most unsophisticated \LM{(less sophisticated ?)}preconditioners is the Jacobi one.
    ...
    \subsection{Evaluating the Jacobian matrices}
      ...
      As the function $\operatorname{G}$ comes from the spatial discretisation method applied to the original partial differential equations (\ref{eq:pde}), it is quite complex to evaluate this Jacobian.%, not in the sense of imaginary numbers but in the sense that it is complicated.
