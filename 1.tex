...
\end{tcolorbox}
  ...
  \section{Problem setup}
    ...
    \paragraph{}
    \GP{In order to set the mathematical framework for this study, let's start from a partial differential equation arising} from the physical model, in the form of
    ...
    \paragraph{}
    In this work, \GP{attention is paid on} computational fluid dynamics %. we will talk about computational fluid dynamics.
    ...
    \GP{solution of the Navier--Stokes equation}, and its variants: the reactive Navier--Stokes equation, the Reynold-averaged Navier--Stokes equation, etc.
    ...
    In our solver CEDRE, this relation is usually more complex as we are working with multifluid flows and with variable heat capacities.
    ...
    Yet it is clear that with a bit of rewriting, \GP{it is possible to get} back to the starting form (\ref{eq:pde}) and even the conservative form with source terms (\ref{eq:pde_conservative}).
    ...
    \GP{For the numerical resolution, the different quantities, such as the state variable $\xi$, are represented discretely 
    over the domain $\mathcal{D}$ and store it in the memory of a computer.}
    ...
    Anyway, \GP{no one can} work in a continuous domain $\mathcal{D}$ but on a discretised one.
    ...
    \end{equation}
    The difference here is that the function $\operatorname{G}$ is a function of a discrete vector whereas $\operatorname{F}$ was a function of a continuous \GP{domain}, and therefore $\operatorname{G}$ does not use any spatial derivatives.
  ...
  \section{Brief introduction to the spatial integration schemes}
    ...
    Indeed, before solving equation (\ref{eq:pde}) \GP{anyone} must decide how to transform the continuous model into a discretised one.
    ...
    \subsection{The Finite Volume method}
      ...
      It tells to store the quantities as the averaged values represented at the center of gravity of each cell as our vector $\Xi$.
      \GP{An important aspect concerns the transformation of} the divergence from equation (\ref{eq:pde_conservative}) \GP{into} a flux surface average, 
      \GP{and the flux computation is still here an open question}.
      ...
      \GP{$\vec{s_{ij}}$ is the product of the face area and the unit normal vector directed ouwards volume $i$.}
      This approximation is a key element of the Finite Volume method and \GP{will be discussed later}.
      \GP{The} function $\operatorname{G}$ from equation (\ref{eq:ode}) \GP{can be computed}: for each face of the mesh \GP{one} computes $\vec{\operatorname{f}}_{ij} \cdot \vec{s_{ij}}$, \GP{adds} this value to the $i$th component and removes it from the $j$th component of the new vector.
      Then, the vector containing the result of $\operatorname{G}\left(\Xi\right)$ \GP{is get after adding the source terms}.
    ...
    \subsection{The Riemann problem}
      ...
      On the interfaces between two cells, the left and right quantities $\xi_L$ and $\xi_R$ are known, and \GP{are input data} to compute the corresponding flux.
    ...
    \subsection{Gradient reconstruction methods}
      ...
      It consists in handling the surface flux evaluation on one hand, \GP{as} in the Riemann problem part, and deciding what left and right quantities to feed to this flux computation on the other hand.
      ...
      \subsubsection{The $k$-exact method}
        ...
        \paragraph{}
        The first method used to reconstruct high-order quantities is called the \emph{$k$-exact} method \GP{with successive corrections}.
        ...
        Creating local extrema in the solution can be troublesome, and so the $k$-exact method \GP{includes limiters to limit the reconstructed data}.
      ...
      \subsubsection{The Multislope method}
        ...
        \GP{This method is a direct adaptation of the structured dirctional method to unstructured grids. It consists in defining local 
        directions for the interpolation of the unknown on the face. For a given cell and one of its face, one can draw the line from those points. 
        Using surrouding cells, the goal is to interpolate information on the line. In practice, it means that two three-point stencils are available. }
        ...
        \GP{The mechanism to define the slopes and the geometric treatment is quite complex and it won't be discussed here. The details 
        are provided in \cite{LeTouzeMurroneGuillard2015}.}
  ...
  \section{Introduction to the time integration methods}
    ...
    \GP{The equation is solved} numerically, which means \GP{the next solution is computed} iteratively after a given time step, knowing the current one.
    It is also possible to modify the equation, as \GP{the interest is} in the final state, not in the transient one.
    ...
    \GP{Let} $\Delta t_n$ \GP{denote} the time step $t_{n+1} - t_n$.
    \GP{Look at a single step of time integration methods in the following, the subscript on the time step can be drop as it is not meaningful}.
    \GP{For} a steady solution, the evolution of the solution \GP{has no interest} and 
    it seems reasonable to want to "go fast" to the steady state, meaning to use as large a time step as possible.
    ...
    \subsection{Analysis of time integration methods}
      ...
      \subsubsection{Stability}
        ...
        \paragraph{}
        \GP{The stability of a time integration method is usually analysed on} the ordinary differential equation with a linear right-hand side \cite{HairerWanner1996}.
        The reason is that if $\tilde{\Xi}$ is solution of equation (\ref{eq:init_value_ode}), $\operatorname{G}$ \GP{can be linearised in} $\tilde{\Xi}$.
        With $y = \Xi - \tilde{\Xi}$ and $J = \frac{\partial \operatorname{G}}{\partial \Xi}\left(\tilde{\Xi}\right)$, assumed constant, \GP{it comes}:
        ...
        \GP{Now, the goal is to compute the eigenvalues and eigenvectors of the matrix $J$} in $\mathbb{C}$.
        ...
        Finally, this is why equation (\ref{eq:dahlquist}) \GP{is studied} assuming the eigenvalues of $J$ are negative.
        ...
        \GP{The initial value $y_0$ can be decomposed} on a basis of eigenvectors of $J$, $v_1, \dots, v_N$, associated with the eigenvalues $\alpha_1, \dots, \alpha_N$: $y_0 = \sum_{i=1}^N \lambda_i v_i$.
        Because $g$ is an analytic function, it \GP{comes immediatly}:
        \begin{equation}
    ...
    \subsection{Explicit methods}
      ...
      \subsubsection{Runge--Kutta methods}
        ...
        Suppose that the value of $\Xi_n$ in $t_n$ is known, the intermediates steps $t_{n, i} = t_n + c_i\Delta t$ for $1 \leq i \leq k$, with a fixed $k$ are set.
      ...
      \subsubsection{Adams--Bashforth methods}
        ...
        The Adams--Bashforth \GP{methods} can reach a high order of accuracy while staying computationally inexpensive, but they drastically lack stability, and that is why they are often not used in computational fluid dynamics computations.
    ...
    \subsection{Implicit methods}
      ...
      \subsubsection{Implicit Euler method}
        ...
        \paragraph{}
        After checking that this is a first-order method, the stability analysis \GP{can be performed} to find the corresponding function $g\left(z\right) = \left(1 - z\right)^{-1}$, that gives a stability region equal to the whole complex plane minus the closed unity disk centered in 1.
      ...
      \subsubsection{Implicit Runge--Kutta methods}
        ...
        Theoretically, this means \GP{that an arbitrarily high order can be achieved} while keeping the stability quality with these methods.
        ...
        Furthermore, if all quadrature coefficient $a_{ii}$ are all equals, as  \GP{the following matrix must be inverted} at each step the matrix
        \begin{equation}
          \operatorname{Id} - a_{ii} \Delta t \frac{\mathrm{d} \operatorname{G}}{\mathrm{d} \Xi}\left(\Xi_n\right) ,
        \end{equation}
        this can help \GP{solving} the linear problem.
      ...
      \subsubsection{Backward differentiation formula}
        ...
        Then, the time derivative of the solution \GP{can be replaced by} the time derivative of the interpolating polynomial \GP{that is evaluated} at the next time step $t_{n+1}$.
        This gives an implicit equation to invert to get $\Xi_{n+1}$.
        ...
        \paragraph{}
        The order of the methods is equal to the index of the methods, corresponding to the number of previous states needed to compute the next one.
  ...
  \section{Implicit methods framework}
    ...
    \subsection{Methodology of the implicit time integration}
      ...
      \subsubsection{From a nonlinear problem \dots}
      	...
      	\end{equation}
        Equivalently, as work \GP{is performed} here for a single $n$ at a time, this equation \GP{can be expressed} in terms of the increment $x = \Xi_{n+1} - \Xi_n$:
        \begin{equation}\label{eq:nonlinear}
      		f\left(x\right) = 0 .
      	\end{equation}
        Newton's method starts from an initial guess $x_0=0$ and it is equivalent to taking $\Xi_n$ as an initial guess for $\Xi_{n+1}$.
        The method will then iterate to approximate the solution of equation (\ref{eq:nonlinear}).

        \paragraph{}
        At each step of Newton's method, a first-order linearisation in the current estimation $x_i$ of the nonlinear function $f$ is evaluated in the next iteration $x_{i+1}$ and this linearisation is set equal to 0, so:
      ...
      \subsubsection{\dots to a linear one}
        ...
        The $x$ notation \GP{is introduced} as it is often used in the literature to name the unknown in linear problems.
    ...
    \subsection{Nonlinear solver}
      ...
      \GP{Since we could benefit from an actual Newton's method, it was implemented, accounting for several Jacobian matrices, because several linear problems need to be solved during one single time step.}
    ...
    \subsection{Linear solver}
      ...
      For the \GP{considered} spatial discretisation methods, a cell only depends on a small number of neighbouring cells, and therefore a degree of freedom is not linked with most of the others.
      ...
      But for problems with huge sizes such as the ones in computational fluid dynamics, the amount of work is too much to be computed with today's means.
      ...
      The corresponding Krylov subspaces are defined as:
  		...
  		\end{equation}
      Then, the next iterate in $\krylov{n}$ \GP{must satisfy} a Petrov--Galerkin condition \cite{SimonciniSzyld2007}:
  		\begin{equation}
      ...
      To handle this case, the GMRES algorithm \GP{must be modified} into the Flexible Generalized minimal residual method, or FGMRES \cite{Saad1993, SimonciniSzyld2002}, \GP{with its associated advantages}.
      %We saw several advantages to this method.
      GMRES \GP{can be preconditioned} with an inner GMRES, and as GMRES was already written in our solver this idea was simple to apply.
