...
\end{tcolorbox}
  ...
  \section{Problem setup}
    ...
    \begin{equation}\label{eq:ns}
      \left\{\begin{aligned}
        ...
        &\partial_t\left( \rho E      \right) &&+ \nabla\cdot\left( \left(\rho E + p\right) \vec{u} \right) &&=
          \nabla\cdot\left( \mat{\tau} \cdot \vec{u} \GP{- \vec{q}}\right)
      \end{aligned}\right.
    \end{equation}
    ...
    \GP{The heat flux term $\vec{q}$ enables heat exchange in the fluid, through the heat diffusivity, and it is generally  
    related with the tempreature gradient.}
    Without \GP{those terms}, one recovers the Euler equations.
    ...
    For a numerical computation, the different quantities, such as the state variable $\xi$, are represented discretely over the domain $\mathcal{D}$ and \GP{they are stored} in the memory of a computer.
  ...
  \section{Brief introduction to the spatial integration schemes}
    ...
    \subsection{The Finite Volume method}
      ...
      In the case of the Navier--Stokes equations (\ref{eq:ns}), the density, the momentum and the energy are conserved throughout time, apart from what comes in and out of the domain \GP{and more generally from the set of boundary conditions\footnote{paroi isotherme à 1700K apporte de la chaleur au fluide... et in et out me fait penser aux directions des ondes hyperboliques...}}.
  ...
  \section{Implicit methods framework}
    ...
    \subsection{Methodology of the implicit time integration}
      ...
      In this thesis, we are interested in the non-hatched parts.%\GP{J'ai pas compris ce qui est non grisé... Aurais-je un souci d'affichage ?}\GP{C'est tout bon maintenant !!!}
    ...
    We explained that this linearisation in CEDRE is done by using Jacobian matrices that are \GP{poor} approximates.
    It means that we do not solve the correct linear systems, which hinders the nonlinear %solve 
    \GP{equation} and the time integration method ends up finding an incorrect increment.
