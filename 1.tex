\chapter{Analysis of existing methods}
  ...
  \section{\PS{Principe des méthodes implicites}}

  	\paragraph{}
	As we explained in the previous section, implicit time integration methods give the next state as the solution of a non-linear equation, or a system of non-linear equations.
	If we want to use implicit methods, we then need to be able to solve a non-linear problem.
	...
	For Diagonally Implicit Runge--Kutta methods, we apply the non-linear solve succesively for each step.
	For BDF methods, we apply the non-linear solve on a slightly different equation.

	\paragraph{}
	We can use a Newton method to solve a non-linear problem of the form:
	\begin{equation}\label{eq:nonlinear}
		F\left(\Xi_{n+1}\right) = 0 \ .
	\end{equation}
	\PS{Newton's ? + Vérifier si fonction F pas déja utilisée}.
	The idea is to linearise equation (\ref{eq:nonlinear}) in $\Xi_n$:
	\begin{equation}
	  F\left(\Xi_n\right) + \frac{\mathrm{d} F}{\mathrm{d} \Xi}\left(\Xi_n\right) x = 0
	\end{equation}
	with the unknown $x = \Xi_{n+1} - \Xi_n$.
