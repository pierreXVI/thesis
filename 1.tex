\chapter{Analysis of existing methods}
  ...
  In particular, we need to explain what performances means.
  ...
  \PS{Revoir structure}
    ...
    \subsection{Nonlinear solver}
      ...
      What was done in our solver was in fact a single step of Newton's method, which means a single linearisation of the nonlinear problem.
      We thought we would benefit from an actual Newton's method, so we implemented it.
      Contrary to what was done before, several linear problems need to be solved during one time step, which means using several Jacobian matrices.
      ...
      \end{equation}
      During this thesis, we did some work towards using a complete Newton's method, but we did not have time to develop a Line Search algorithm, and so we ended up using the standard method from our solver: a single linearisation of the nonlinear problem.
    ...
    \subsection{Linear solver}
      ...
      This allows for some control on the residual norm.
      ...
      The condition number of an inversible matrix $A$ is defined as:
      \begin{equation}
        \kappa\left( A \right) = \norm{A} \norm{A^{-1}}
      \end{equation}
      ...
      If this help picture what the condition number stands for, this is only true for normal matrices: matrices that commute with their conjugate transpose.
      ...
      In other words, there are ill conditioned matrices for which GMRES converges quickly, and well conditioned matrices on which GMRES will be slow.
      ...
      To handle it in the analysis on the convergence, one must not look at the spectrum but the pseudospectrum \cite{Trefethen1999}.
      ...
      As the matrices we encounter in our computational fluid dynamic problems are \PS{quelconques}, meaning \PS{à priori non-normales}, we decided not to study finely the spectrum of the operator.

      \paragraph{}
      Even if we will not look deeply into the spectrum of the matrix, we still to help to the linear solver with preconditioning.
      ...
      \end{equation}
      The idea is to transform $A$ into a matrix that is more easily inversible.
      The matrix we need to invert is now $PA$ for left preconditioning and $AP$ for right preconditioning.
      If the preconditioning operator is close to $A^{-1}$ and is cheap to compute, this new matrix is close to $\operatorname{Id}$ and so is easily inversible.
      ...
      Choosing a preconditioning means making a compromise between those two extrema.
      ...
      \paragraph{}
       To better understand how preconditioning works, let us take an example, inspired from example 35.2 of \cite{TrefethenBau1997}.
       ...
       This helps a lot GMRES, as can be seen on figure \ref{fig:preconditioning}: it struggles to reduce the residual norm on the standard problem, but does it easily on the preconditioned problem.
       This is only a dummy example, but it helps visualise the importance of preconditioning.
   		...
   		\begin{figure}
   			...
   			\caption{Spectrum of $A$ in blue and $A_{pre} = AD^{-1}$ in orange (left). GMRES convergence pour those matrices (right).}
      ...
      On the other hand, it lacks of efficiency, and that is why many preconditioners were developed in the literature.
      A slightly variation consists in taking the diagonal blocs instead of the diagonal elements.
      This makes sense for our matrices, as they can be divided in blocs, each bloc corresponding to the degrees of freedom in a cell.
      ...
      \paragraph{}
      Others preconditioners exists in our solver.
      ...
      As it is even simpler than the Jacobi preconditioner, it shows poor performances.
      Some work was done towards Polynomial preconditioning: the preconditioning matrix $P$ is equal to the partial Neumann serie of $\operatorname{Id} - A$, as it approximates $A^{-1}$ \cite{DuboisGreenbaumRodrigue1979}.
      Its major drawback is its computational cost, as it require multiple matrix vector products.
      ...
      When taking the LU factorisation of the sparse matrix $A$, the triangular matrices $L$ and $U$ loose the sparsity pattern of $A$.
      ...
      This way, the factorisation in incomplete in the sense that it does not recreates $A$, but it is still sparse and can be used as a preconditioner.
      ...
      \paragraph{}
      As we do not need explicitly the preconditioning matrix $P$, and its effet should be near $A^{-1}$, one could take as a preconditioning procedure another Krylov subspace method:
      ...
      This preconditioning does not need more information on the matrix $A$ than the outer GMRES does: in particular it does not require to know the matrix coefficients.
      This is interesting to us for some reasons that are discussed lated.
      ...
      Restarting allows for some control on the Krylov subspace dimension, and therefore on computational cost and memory usage.
    ...
    \subsection{Evaluating the Jacobian matrices}
      ...
      \paragraph{}
      If we can solve precisely linear problems, but the problem is not the one the nonlinear solver want, it may hurt the nonlinear solver convergence.
      ...
      As this derivative is the matrix we use when solving the linear problem, is is crucial to have a good representation of it.
      ...
      Furthermore, our software keeps evolving, and models are constantly added and modify.
      It would require a constant work to maintain the Jacobian matrix computation.
      ...
      Using this, and the fact that the matrix in a Jacobian matrix, we could approximate the matrix vector product of $f'\left(x\right)$ with a vector $v$ by:
      ...
      \begin{equation}
        f'\left(x\right) v \approx \frac{f\left(x + \varepsilon v\right) - f\left(x - \varepsilon v\right)}{\varepsilon}
      \end{equation}
      ...
      If we apply the approximation (\ref{eq:matrix_free}) for each vector of the \PS{base canonique} as the vector $v$, we can gather each columns of the Jacobian matrix.
      This gives what is often called the finite difference Jacobian matrix.
      Instead of computing a matrix vector product for each direction, a technique consists in computing independent directions beforehand to get a coloring of the matrix: two directions are of the same color if they are independent through the matrix.
      Then, a smaller number of matrix vector product approximations are required: as much as there are colors.
      This is often called the finite difference Jacobian matrix with coloring \cite{GebremedhinMannePothen2005}.
      ...
      As it represent the size of the step made to approximate a derivative, it should be small.
      ...
      Working with different Jacobian matrices is not an issue, as they are not actually computed.
      The Krylov subspace method GMRES does not need the Jacobian matrix, only to compute matrix vector products.
      ...
      Instead, in CEDRE, each solver iterate independently to the others, with some exchanges between iterations.
