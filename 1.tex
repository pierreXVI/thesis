  ...
  \section{Time integration methods}
    ...
    \subsection{Analysis of time integration methods}
      ...
      \subsubsection{Consistency and order}
        ...
        To convince oneself, we could say that when solving the differential equation \PS{on/in} an interval $\left[0, T\right]$ with a fixed $T$, the global error of a $p$ order method would \PS{be of, behave like ?} $O\left(\Delta t^p\right)$ as it amount to summing $T/\Delta t$ local errors of $O\left(\Delta t^{p+1}\right)$.
