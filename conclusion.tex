...
\markboth{}{CONCLUSION AND PERSPECTIVES}
  ...
  \section*{Jacobian-Free Newton--Krylov for implicit time integration}
    ...
    \paragraph{}
    \GP{One key argument of the Jacobian-Free Newton--Krylov is the possibility to handle the Jacobian approximation at the same order of accuracy as the spatial discretization. We showed that 
    accuracy can lead to an improved comvergence, or at least, has a limited impact on convergence. 
    Rough approximations of the Jacobians should be avoided in the future, or considered only for quick tests. 
    If the choice of CEDRE developers is finally ro revert to a Jacobian-based formulation, we can assume that 
    the computation of the Jacobian will be a key ingredient. Doing this by hand is time-consumming and can lead 
    to implementation issue. An alternative could be considering code differentiation. But in the context of CEDRE, it means revisiting the full structures of the whole set of solvers. 
    }
  ...
  \section*{Exponential integration methods for unsteady time integration with large time steps}
    ...
    The same idea applies to \GP{thermics} for which large physical times need to be simulated.
