...
\markboth{}{CONCLUSION AND PERSPECTIVES}
  ...
  \section*{Jacobian-Free Newton--Krylov for implicit time integration}
    ...
    We showed that accuracy can lead to an improved comvergence, or at least, \PS{has a limited impact on convergence}.
    ...
    Doing this by hand is time-consumming and can lead to implementation issues.
    An alternative could be considering code differentiation.
    \PS{But in the context of CEDRE, it means revisiting the full structures of the whole set of solvers.}
  ...
  \section*{Exponential integration methods for unsteady time integration with large time steps}
    ...
    The same idea applies to thermics for which large physical times need to be simulated.
