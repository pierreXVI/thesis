  ...
  \section{Utilisation de la formulation sans matrice sur un nouveau modèle de fluide}
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Full reactive model}
        ...
        Then, we continue the computation with the JFNK method using the first-order Finite Volume method, to quickly get a good enough approximation of the expected solution.\LM{Pour les premières itérations avec la "midpoint method", on est à l'ordre 1 en espace ou pas? Et pour le passage au JFNK, l'ordre 1 c'est pour tout le schéma spatial, ou juste pour l'approximation de l'action de la matrice?}
        Finally, to get a finer result, we end the simulation using the second-order spatial discretisation method.
        We use local time-stepping based on the CFL number.\LM{je ne sais plus si c'est possible, mais il y a-t-il du pas de temps local pour les simulations "explicites"}
