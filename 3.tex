  ...
  \section{Utilisation de la formulation sans matrice sur un nouveau modèle de fluide}
    ...
    \subsection{Multi-TEmperature model}
      ...
      \begin{itemize}
        ...
        \item the \emph{light} ones that are at \PS{vibrational disequilibrium}
        \item the electrons that are handled separately.
      \end{itemize}

      \paragraph{}
      To account for the disequilibrium, the Navier--Stokes equations are modified into the following, in conservative form.
      \begin{equation}
      ...
      The variables $y$ correspond to the mass density of the different flow components.
      ...
      Some work tried to transform this equation into a conservative one, by using the electronic entropy instead of energy, but it appears that using this nonconservative formulation is necessary to get physically accurate results \PS{SOURCE JM LAMET}.
      ...
      This is interesting on reentry problems, as there is indeed a non-equilibrium behind the strong shocks that appears on such problems.
      Taking the non-equilibrium into account can change the flow behind the shock, the thermal exchanges or other physical features.
      Computing the electronic density can also be useful when studying \PS{signature electromagnetique}.
      Generally speaking, the Multi-TEmperature model gives more precise results than the standard Navier--Stokes model on reentry problems or \PS{forte détente} problems, where there may be some thermodynamic non-equilibrium.
