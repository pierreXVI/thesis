...
\begin{tcolorbox}[title=Résumé du chapitre : Analyse de la méthode JFNK dans CEDRE, colframe=black!50!white]
  ...
  Pour finir, nous utilisons un modèle de fluide récemment ajouté au solveur afin de représenter \GP{plus finement} le déséquilibre thermodynamique, \GP{en incluant plus d'effets thermiques}\footnote{J'essaye d'introduire la notion de multi température}.
...
\end{tcolorbox}
  ...
  \GP{Version 2: Full-Jacobian approximation against Matrix-Free formulation}
    ...
    \subsection{Turbulent transonic airfoil}
      ...
      \subsubsection{Analysis of the results}
        ...
        With the Spalart--Allmaras compressible model, the conservative variable \GP{is} in fact the product of the density and the Spalart--Allmaras variable $\rho \tilde{\nu}$.
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Definition of the test case}
        ...
        \GP{It} is indeed seen in figure \ref{fig:sphere_fields} on the pressure and temperature.
      ...
      \subsubsection{Analysis of the results}
        ...
        \PS{Comparaison distance corps shock avec \cite{Lobb1964} ?}\GP{Si tu veux, cela apporte de la physique. Mais ce n'est pas une obligation car on peut partir du principe que Charme est validé...}

        \paragraph{}
        To analyse the result from this case, one might once again look at the residuals.
        The first result correspond to a \GP{computation} without reactive model.
        ...
        Because it uses a better Jacobian matrix in the linear solve, it gives a better solution to the nonlinear \GP{problem}\footnote{\GP{eviter repetition de solve}}, which gives in turn a better time integration.
  ...
  \section{Utilisation de la formulation sans matrice sur un nouveau modèle de fluide}
    ...
    This is why the new models can not compute \GP{yet} Jacobian matrices, and therefore can not use implicit methods.
    ...
    \subsection{Multi-TEmperature model}
      ...
      \GP{Je crois que ce qui gene le plus est "but by neglecting dissipation terms concerning the electrons". Tu veux dire que les termes de dissipation sont supprimés ou modélisés différemment ? }
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Simplified reactive model}
        ...
        With the full reactive model, the limiting factor for the implicit method came from the stiffness due to the electrons.\LM{On est sûr que c'est la raideur, ou c'est le fait que l'on patche, lorsqu'il n'y a plus d'électrons, par un raccord non C1?}\GP{Lionel, je comprends pas ce que tu sous entend... Il faudra qu'on clarifie après les vacances...}
