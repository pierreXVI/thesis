...
\end{tcolorbox}
  ...
  \GP{Comparison between full-Jacobian matrix and matrix-free formulations}
    ...
    \paragraph{}
    \footnote{\PS{J'avais commencé par "From our analysis and implementation," pour faire une transition et dire que ça découle des chapitres précédents. Je trouve que là ça fait "vérité générale"}}
    ...
    In this part, the new method that uses the matrix-free approach will be compared with the implicit Euler method as implicit time integration schemes.\footnote{\PS{Au lieu de "as we are interested in implicit time integration", je trouve que sans ça il y a moins d'enchaînement logique}}
    ...
    \subsection{Turbulent transonic airfoil}
      ...
      \subsubsection{Analysis of the results}
        ...
        For example, the residual 2-norm associated with the $i$th component is: \LM{J'avoue que mes souvenirs sont assez vague, mais il me semblait que la norme L2 était la racine de l'intégrale des carré; ce que tu écris ne serait pas la norme L1 plutôt?}
        \begin{equation}
        ...
        \end{equation}
        In this fluid dynamics application, the residual norm is associated with the conservative variables: the density, the momentum components, the energy and the turbulent variable from the Spalart--Allmaras model $\nu_t$.
        \PS{C'est quoi la variable conservée (facteur $\rho$) ?}

        \begin{figure}
          \centering
          \includegraphics{figures/rae_residuals.png}
          \caption{Residual 2-norms for the two momentum components, energy and turbulent viscosity throughout the computation.}
    ...
    \subsection{Turbulent transonic airfoil with boundary layer resolution}
      ...
      \subsubsection{Analysis of the results}
        ...
        \paragraph{}
        As before, the computation starts from an initial state constant in the domain and the scheme..
        ...
        The residuals are given in figure \ref{fig:rae_residuals_fine_l2}.
        ...
        \paragraph{}
        Figure \ref{fig:rae_residuals_fine_l2} shows that the reduction in residual 2-norms with the standard method is not satisfying.
        ...
        \begin{figure}
          \centering
          \includegraphics{figures/rae_residuals_fine_l2.png}
          \caption{Residual 2-norms for the horizontal and vertical momentum components, energy and turbulent viscosity throughout the computation.}
          \label{fig:rae_residuals_fine_l2}
        \end{figure}
        ...
        Note that the width of the curve is due to data noise, as all lines use the same width.
        \footnote{\PS{Ca me fait bizarre "Note that", ça "apostrophe"}}

        \begin{figure}
          \centering
          \includegraphics{figures/rae_residuals_fine_linf.png}
          \caption{Residual $\infty$-norms for the horizontal and vertical momentum components, energy and turbulent viscosity throughout the computation.}
          \label{fig:rae_residuals_fine_linf}
        \end{figure}

        \paragraph{}
        The value of the turbulent variable $\tilde{\nu}$ in the first cell above the wing profile is provided for both computations in
        figure \ref{fig:rae_field_fine}.
        ...
        Figure \ref{fig:rae_residuals_fine_linf} shows that the $\infty$-norm of the residual on the conservative variable $\rho \tilde{\nu}$ is near $10^2$ \PS{TODO: pas d'unité}, so figure \ref{fig:rae_field_fine} highlights cells for which the residual is higher than $10^{1.8}$.
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Definition of the test case}
        ...
        \GP{Je ne mettrais pas ça :
        Otherwise, artificial vorticity is created when going through the shock, which will give wrong results downstream.}
        \PS{Tu reformulerais ou tu enlèverais ?}
        \LM{Est-ce que c'est l'aspect ratio ou le fait que les faces soient orientées selon le choc qui nous sauve ici.. A la limite, si on avait la possibilité de mailler hyper-fin avec des mailles isotropes alignées sur le choc, on aurait quand même de la vorticité artificielle?}
      ...
      \subsubsection{Analysis of the results}
        ...
        \GP{This is perhaps a consequence of the fact that the convergence rate is enough with the standard implicit method to reach the steady state.}
        \GP{Faudra-t-il laisser cela ???
  ...
  \section{Utilisation de la formulation sans matrice sur un nouveau modèle de fluide}
    ...
    For example, a new model was recently added to better handle multiphasic flows \cite{Cordesse2020}. \LM{mais du coup tu ne l'utilises pas du tout ce nouveau modèle dans la suite, non?}
    ...
    This is why the new models can not compute Jacobian matrices, and therefore can not use implicit methods.\LM{Je dirais plutôt qu'on ne se précipite pas sur l'effort d'évaluation du jacobien, pas qu'on ne peut pas l'évaluer, non?}
    ...
    \subsection{Multi-TEmperature model}
      ...
      \paragraph{}
      To account for the disequilibrium, the Navier--Stokes equations (\ref{eq:ns}) are modified into the following=\GP{Il faut virer ce qui suit car la formulation est non conservative..., in conservative form.}
      \PS{Virer que "in conservative form" ou virer toute l'équation ?}
      \begin{equation}
      ...
      Some work tried to transform this equation into a conservative one by using the electronic entropy instead of energy but by neglecting dissipation terms concerning the electrons \cite{CoquelMarmignon1995}.\LM{Je ne comprends pas la phrase..}
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Full reactive model}
        ...
        \begin{figure}
          ...
          \caption{Residual norms for the electron mass fraction, vertical momentum, \ce{N_2} vibrational energy and total energy for the Multi-TEmperature model computation.}
