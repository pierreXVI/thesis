  ...
  \section{Comparaison entre la matrice jacobienne explicite et la formulation sans matrice}
    ...
    \subsection{Turbulent transonic airfoil}
      ...
      \subsubsection{Analysis of the results}
        ...
        Finally, we can look at the aerodynamic coefficients, the drag and lift coefficients, throughout the simulation on figure \ref{fig:rae_coefficients}.
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Definition of the test case}
        ...
        \paragraph{}
        The 2D mesh and is shown in figure \ref{fig:sphere_mesh}.
        ...
        The refinement at the wall boundary helps to compute more precisely the physical phenomena that happen in said boundary.
        ...
        Otherwise, artificial vorticity is created when going through the shock, which will gives wrong results downstream. \PS{c'est bien pour ça ?}
        ...
        \paragraph{}
        The solid sphere is modelised by a isothermal wall boundary condition.
        This is a representative choice as well, as the heat flux going through the wall is one of the main interest of such computation.
        ...
        Indeed CEDRE do not try to be a pure aerodynamic solver but a multiphysics one, equipped to solve high energy problems.
        Reentry phenomena are therefore in the scope of our solver.
        Any improvements coming from our method on such cases would benefit a lot of CEDRE users on their applications.
      ...
      \subsubsection{Analysis of the results}
        ...
        On figure \PS{TODO}, we see the shock, and that is falls as expected in the refined region.
        We see on figure \PS{TODO} the mass fraction of the various species downstream of the shock, which means we are actively computing the chemical features of the flow, as expected.
        \PS{Comparaison distance corps shock avec \cite{Lobb1964} ?}

        \paragraph{}
        Once agin, we will not see any difference between the two results if we look at the \PS{standard} quantities.
        ...
        This shows an inacurracy of the solver.
        However, as we will show next, the time integration is able to find the correct converged steady solution.
        Therefore this inacurracy comes from the spatial discretisation method.
        The results are not acurate from a physical analysis perspective, but for us the methods both work fine: they find the expected steady solution, albeit it is not the expected physical one.
        ...
        The difference between the traditional method and the new one is that the traditional method uses the Jacobian matrix of the first order spatial discretisation method, wheras the new one approximates the true Jacobian matrix using the approximation \ref{eq:matrix_free}.
        ...
        We look at the residual norms from this method, that we will call the first order JFNK method, and we see in figure \PS{TODO} that is is undistinguishable from the residual norms from the traditional method.
        It first validates the developpement of the matrix-free approximation.
        ...
        When we use the new method, using the true function $f$ with a second-order, we see that we are able to get a much smaller residual norm.
