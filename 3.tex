...
\end{tcolorbox}
  ...
  \section{Using the matrix-free formulation with a new fluid model}
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Simplified reactive model}
        ...
        The computation starts once again with a few iterations of the Midpoint method and use that as a common initialisation.
  ...
  When compared to the traditional implicit method on turbulent RANS computations, it showed that the JFNK achieves better convergence in the residual norms, when the convergence of traditional method is unsatisfactory.
  The conclusion is similar on typical reentry applications.
  When the traditional method is able to converge well, however, the result is no longer in favor of the JFNK method, although the difference between the two method is not significant.
  Finally, the matrix-free method aws used with a newly implemented fluid model that does not give access to a Jacobian matrix yet.
  For this reason, using the JFNK method is the only way of using an implicit method.
  It was compared to the explicit Midpoint method that is currently used for computaions with this fluid model.
  The results are once again in favor of the JFNK method, both in terms of convergence and speed.
  The superiority of the matrix-free method depends on the level of complexity of the model: the required time to reach the same convergence in residual norms is divided by 2 with the full model, and by 4 with the model without ionisation.
