...
\end{tcolorbox}
  ...
  \section{Using the matrix-free formulation with a new fluid model}
    ...
    \PS{Another model that we did investigate was added to better account for thermodynamic disequilibriums.}
    \footnote{
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Simplified reactive model}
        ...
        \PS{Here, the term stifness corresponds to both stifness from the equation and difficulties arising from the implementation, that uses tools such as \emph{min/max} functions that adds non-differentiability.}
        \footnote{
  ...
  The conclusion is similar with typical reentry applications.
  When the traditional method is able to converge well, however, the result is no longer in favour of the JFNK method, although the difference between the two methods is not significant.
