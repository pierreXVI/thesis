...
\end{tcolorbox}
  ...
  \GP{Comparison between full-Jacobian and matrix-free formulations}
    ...
    \subsection{Turbulent transonic airfoil with boundary layer resolution}
      ...
      \subsubsection{Analysis of the results}
        ...
        We then look once again at the residuals in figure \ref{fig:rae_residuals_fine_l2}.
        ...
        \paragraph{}
        We see in figure \ref{fig:rae_residuals_fine_l2} that the reduction in residual 2-norms with the standard method is not satisfying.
        ...
        \PS{
        \paragraph{}
        In figure \ref{fig:rae_residuals_fine_linf} we see residual norms for the same simulations, but this time using the $\infty$-norm.
        This time, we see that the traditional method is not able at all to reduce residual norms.
        ...
        We note that the width of the curve is due to data noise, as all lines use the same width.
        }
        ...
        \PS{
        \paragraph{}
        In figure \ref{fig:rae_field_fine} we see the value of $\tilde{\nu}$ on the profile for both computations.
        The top views show the upper part of the airfoil, and we see a discontinuity due to the shock, as expected.
        ...
        We see that this method produces a solution of lower quality, as the pattern we see is not physical.
        In particular, there are cells on the wall for which $\tilde{\nu}$ is null, which should not happen with our standard Spalart--Allmaras model.
        We see in figure \ref{fig:rae_residuals_fine_linf} that the $\infty$-norm of the residual on the conservative variable $\rho \tilde{\nu}$ is near $10^2$ \PS{pas d'unité ?}, so we highlight on figure \ref{fig:rae_field_fine} cells for which the residual is higher than $10^{1.8}$.
        We see that those cells correspond to the ones for which $\tilde{\nu} = 0$.
        It shows a default of the standard time integration method, as it has non-physical features in the solution that prevent the residual convergence.
        Tn contrast, it shows the quality of the JFNK method on this application.
        }
