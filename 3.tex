...
\end{tcolorbox}
  ...
  \section{Comparison between matrix-free formulation and Jacobian matrix approximation}
    ...
    \subsection{Turbulent transonic airfoil with boundary layer resolution}
      ...
      \subsubsection{Analysis of the results}
        ...
        Overall, it takes much more iterations to decrease residual norms \reviewers{for} this test case compared to the previous one.
        ...
        It shows a flaw \reviewers{in} the standard time integration method, as it has non-physical features in the solution that prevent \reviewers{residual convergence}.
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Definition of the test case}
        ...
        It is a regular mesh made of quadrangles, with refinement in the radial direction at the wall boundary \reviewers{and the} expected shock location.
  ...
  \section{Using the matrix-free formulation with a new fluid model}
    ...
    \reviewers{Another model that we will investigate in the following was added to better account for thermodynamic disequilibriums.}
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Simplified reactive model}
        ...
        \reviewers{The computation starts once again with a few iterations of the Midpoint method.}
        ...
        \reviewers{Here, the term stiffness corresponds to both stiffness from the equation and difficulties arising from the implementation, that uses tools such as \emph{min/max} functions that add non-differentiability.}
        Without them, the convergence is significantly faster.
        \reviewers{However, the time step for the explicit method} is under the same limitations with both reactive models.
  ...
  The conclusion is similar \reviewers{when working} with typical reentry applications.
  When the traditional method \reviewers{can} converge well, however, the result is no longer in favour of the JFNK method, although the difference between the two methods is not significant.
