\chapter{Analyse de la méthode JFNK dans CEDRE}
  ...
  \section{Comparaison entre la matrice jacobienne explicite et la formulation sans matrice}
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Definition of the test case}
        ...
        For exemple, the user can choose whether to interpolate the mass fraction $y_i$ for the species at the cell interfaces for the MUSCL scheme, or the \PS{mass concentration} $\rho y_i$.
  ...
  \section{Utilisation de la formulation sans matrice sur un nouveau modèle de fluide}
    ...
    For example, a new model was recently added to better handle multiphasic flows \PS{ref Clément}.
    Adding a new model amount to writing the function $\operatorname{F}$ from equation (\ref{eq:pde}), or equivalently $\operatorname{G}$ from equation (\ref{eq:ode}) \PS{vérifier labels}.
    ...
    \subsection{Multi-TEmperature model}
      ...
      \PS{Vérifier anglais: transition/al, rotation/al, ...}
      ...
      \begin{itemize}
        ...
        \item the ones that may be at \PS{vibrational disequilibrium}
      ...
      The source terms $\omega$ correspond to \PS{production et destruction chimique}.
      ...
      Some work tried to transform this equation into a conservative one by using the electronic entropy instead of energy\cite{CoquelMarmignon1995}, but it appears that using this nonconservative formulation is necessary to get physically accurate results \PS{SOURCE JM LAMET}.
      ...
      Computing the electronic density can also be useful when studying \PS{signature electromagnetique} or handling particles' electric charges.
      Generally speaking, the Multi-TEmperature model gives more precise results than the standard Navier--Stokes model on fast flow with low relative density where there may be some thermodynamic non-equilibrium, such as reentry problems or \PS{forte détente} problems.
    ...
    \subsection{Hypersonic reactive sphere}
      ...
      \subsubsection{Full reactive model}
        ...
        We of course use the Multi-TEmperature model, and we consider that \ce{N_2} and \ce{O_2} may be at \PS{vibrational disequilibrium}.
        \PS{Pourquoi que ces deux ?}
        This amount to having four temperatures to describe the flow: $T_e$ for the electrons, $T_{v, \ce{N_2}}$ and $T_{v, \ce{O_2}}$ for the vibration modes of \ce{N_2} and \ce{O_2}, and $T$ for the rest of the energy.
        ...
        We use local time-stepping based on the CFL number \PS{Donner le CFL et est-ce qu'on peut pas faire mieux ?}.
        ...
        The $x$-axis will then be the CPU time: the actual time it took to get at the current residual.
        ...
        The fact that implicit method can use larger time steps is not new, but with this specific model it was not possible to use implicit methods until we added the matrix-free approximation.
      ...
      \subsubsection{Simplified reactive model}
        ...
        \paragraph{}
        The equation concerning electrons in the Multi-TEmperature model bring a lot of issues.
        ...
        For example, as the equations hold true everywhere in the numerical domain, they hold true in particular in areas without free electrons, upstream of the shock in our hypersonic sphere for example.
        ...
        We will not go into such implementation details as it goes beyond the scope of this work, but the point is that having electrons is quite troublesome for the numerical methods.
        According to users that work on such reentry applications, it is not always meaningful to compute the fluid ionisation.
        ...
        \paragraph{}
        We decided to do the same comparaison as before but this time with the simplified reactive model.
        ...
        The result is similar to the previous one, except it is even more in favor of the JFNK method.
        ...
        The gain in CPU time is then huge, in favor of the JFNK method.
