\chapter{Analyse de la méthode JFNK dans CEDRE}
  ...
  \section{Comparaison entre la matrice jacobienne explicite et la formulation sans matrice}
    ...
    \subsection{Turbulent transonic airfoil}
      ...
      \subsubsection{Definition of the test case}
        ...
        Figure \ref{fig:rae_mesh} shows that the mesh near the wall boundary is not particular fine, and therefore it is not fine enough to compute the boundary layer.
        It is because this computation uses a turbulence wall modelling.
        ...
        Turbulence wall modelling may be nonconventional in aerodynamic simulations but it is a commonly used feature of our solver, so it is interesting to see how our new method behaves on such case.
      ...
      \subsubsection{Analysis of the results}
        ...
        The blue curve corresponding to the traditional method struggle to converge, and seems to oscillate periodically.
        In comparison, the oscillation of the orange curve is dampened: this correspond to convergence.
        The scale of the oscillation seen in figure \ref{fig:rae_coefficients} is not significant physically speaking, as it it negligible.
        It shows however an issue with the traditional implicit method of the solver CEDRE: it fails to reach proper convergence but it oscillate around the solution.
        The new JFNK method however achieve proper convergence.
  ...
  \section{Utilisation de la formulation sans matrice sur un nouveau modèle de fluide}
    ...
    \subsection{Multi-TEmperature model}
      ...
      Not to go into too much details, such models that account for thermodynamic disequilibrium ate still under discussion as of today \cite{BlancoJosyula2020}.
      ...
      Taking this non-equilibrium into account helps to compute more precisely physical quantities such as the electronic density, which can be useful for magnetohydrodynamics applications.
      ...
      Generally speaking, the Multi-TEmperature model gives more precise results than the standard Navier--Stokes model on fast flow with low relative density where there may be some thermodynamic non-equilibrium, such as reentry problems or strong rarefaction \PS{(forte détente)} problems.
