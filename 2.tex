...
\end{tcolorbox}
  ...
  \section{General description of CEDRE}
    ...
    \begin{itemize}
      ...
      \item THERMOLIB is used to compute the different thermophysical properties such as heat capacities, chemical reaction rates, etc. \PS{Lionel je dis pas de bêtise sur les coefficients de réaction chimique ?} \LM{Si! La thermolib n'est directement appelée par la chimie que pour l'évaluation éventuelle des constantes de réactions inverses via la constante d'équilibre... Mais c'est anecdotique par rapport à son usage principal qui est la fermeture $\rho(P,T,Y_i)$ et $e(P,T,Y_i)$}
    \end{itemize}
    ...
    When CHARME needs to compute the ordinary differential equation function, it is BIBCEDRE that does the MUSCL reconstruction \PS{c'est bien ça ?} \LM{En fait BIBCEDRE évalue les gradients ou les pentes, mais l'interpolation aux faces est faite par le solveur...} so that the Riemann solver can compute the interface flux.
...
\end{minted}
    ...
    \subsection{Linear system normalisation}
      ...
      The diagonal coefficients of the matrix $\mathcal{N}$ are the invert of an estimate of the order of magnitude of the corresponding degree of freedom. \PS{clair ? beaucoup de of}
      ...
      \begin{equation}
        \left( \mathcal{N}^{-1} f'\left(x\right) \mathcal{N} \right) v \approx \mathcal{N}^{-1} \frac{f\left(x + \varepsilon N v\right) - f\left(x\right)}{\varepsilon} \ .
      \end{equation}
