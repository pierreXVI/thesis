...
\end{minted}
      ...
      Some even claim that it can lead to a less expensive method in terms of time \PS{et les refs ?}.
  ...
  It is important to remind oneself that CEDRE is an industrial sized software system, compared to smaller solvers developped for academic purposes.
  The scale of the solver was a limiting factor during this thesis.
  ...
  Moreover, because of the large number of developers that are working on the solver, and the fact that many developements are small tweaks from occasional developers such as interns and PhD students, fixing bugs took a decent amount of time.
  Overall, this explain why we decided to implement the solutions previously discussed.
  They are already well known from the literature, and feels like a natural step up from the already existing methods of CEDRE.
  We would probably have taken a different route if we had work with a different solver.
