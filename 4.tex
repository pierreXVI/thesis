...
\end{tcolorbox}
  ...
  Most of what is done towards the steady problem %\GP{je vire: solve, me semble bof en anglais et n'apporte rien} 
  can therefore be reused for unsteady problem with large time steps.
  ...
  \GP{Je propose ca: But as both time parallel integration methods and time spectral methods do not follow the standard split form (single timeline and parallel space integration) of the equation, they need deep modifications in the solver, someting that should be avoided for large-scale industrial solvers.}
  ...
  \section{Exponential integration methods}
    ...
    \paragraph{}
    \GP{Let us} focus on methods known as exponential methods.
    Despite being known for a long time \cite{Pope1963}, they were not widely used in computational fluid dynamics, because of some difficulties that will explained later, but still interested some scientists \cite{EdwardsTuckermanFriesnerEtAl1994}.
  ...
  \section{Exponential Rosenbrock--Euler method}
    ...
    \subsection{Definition of the exponential Rosenbrock--Euler method}
      ...
      \paragraph{}
      \GP{The exponential Euler method is the most basic one to understand exponential integration methods.}
      ...
      \paragraph{}
      In this method, the linearisation $L = f'\left(y_n\right)$ \GP{is assumed}.
    ...
    \subsection{Analysis of the exponential Rosenbrock--Euler method}
      ...
      \begin{equation}
        ...
        \end{aligned}
      \end{equation}
      \GP{The decomposition chosen earlier is now justified}: if $L = f'\left(y_n\right)$ and therefore $N'\left(y_n\right) = 0$ then $y\left(t_{n+1}\right) - y_{n+1} = O\left(\Delta t^3\right)$ and the method is of order 2.
  ...
  \section{Exponential Runge--Kutta and Rosenbrock methods}
    ...
    It is reasonable to want multiple-stage methods to have a higher order than the single-stage exponential Euler method.
  ...
  \section{Evaluating matrix functions}
    ...
    \subsection{Evaluating matrix exponentials}
      ...
      \paragraph{}
      With the Krylov subspace method just described, the exponential of the smaller Hessenberg matrix $H_m$ 
      \GP{can be computed and attention is now paid on} 
      how to compute the exponential of relatively small dense matrices.
