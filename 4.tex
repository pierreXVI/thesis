\chapter{Analysis of a new type of time integration methods}
  ...
  \paragraph{}
  \PS{Mettre dans une section à part sur le coût ?}
  ...
  The implicit Euler method, that is also a one stage method, also computes a Jacobian matrix, does an Arnoldi decomposition.
  It then invert the Hessenberg matrix with a QR decomposition which is equivalent to applying a function to the Hessenberg matrix.
  ...
  It means shorter Arnoldi iterations, which means a smaller Hessenberg matrices, and overall a much smaller computational cost.
  We explained why the one stage exponential method cost is equivalent to the one stage implicit method cost.
  Because we use the formulation (\ref{eq:exprb_defect}) and the fact that the defects are smalls, adding more stages to the exponential method will be less expensive than adding stages to the implicit method.
  ...
  \section{First developments in CEDRE}
    ...
    The vortex is convected in a flow at a Mach number of $\operatorname{Ma} = 0.5$, with the dimensionnement parameters $P_\infty = 1$ and $T_\infty = 1$.
    ...
    To reduce the number of variables in this comparison, both will use the traditional Jacobian matrices that is the low-order approximation.
    ...
    However, the simulation are made at a CFL number of $0.9$.
    ...
    Finally, the exponential method is both able to handle the higher CFL number and to preserve the vortex well enough.
    It has the robustness of the implicit method and the precision of the higher-order explicit method.

    \paragraph{}
    The analysis on this simple test case is quite rudimentary.
    Indeed, we did not look finely at the error introduced bu the time integration methods, but we visually compared the results.
    ...
    We see that it looses in intensity, and the contour lines are not exactly concentric.
