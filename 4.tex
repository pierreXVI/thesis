...
\end{tcolorbox}
  ...
  \section{Exponential integration methods}
    ...
    \reviewers{They are still actively studied \cite{NarayanamurthiSandu2020, CaliariCassiniZivcovich2023}.}
  ...
  \section{Exponential Runge--Kutta and Rosenbrock methods}
    ...
    \end{equation}
    \reviewers{It is reasonable to want multiple-stage methods to have a higher order than the single-stage exponential Euler method, and therefore it must also have}
    \begin{equation}
    ...
    \reviewers{For both methods, the coefficient that is the hardest to get is $b_1$, however equation (\ref{eq:exprb_defect}) tells us that it will not be computed.}
  ...
  \section{Evaluating matrix functions}
    ...
    \subsection{Evaluating matrix exponentials}
      ...
      Attention is now paid \reviewers{to} how to compute the exponential of relatively small dense matrices.
  ...
  \section{First developments in CEDRE}
    ...
    \reviewers{The solution for each method is shown} in figure \ref{fig:covo_cedre_fields}.
